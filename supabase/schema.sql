...
-- It is idempotent for new projects. Re-running on a populated
-- database will fail on CREATE TABLE — wrap blocks in IF NOT EXISTS
-- yourself or run only what changed.
-- ============================================================
...
-- ============================================================
-- GRANTS — public schema does not grant by default
-- ============================================================
...
-- Drivers (using the RPC) are anonymous PostgREST callers; they need to be able to
-- read vehicles/sessions and write inspections/damage. We allow this via RLS below.
...
-- Drivers (anonymous role used by PIN-authed drivers): minimal needed access
-- They authenticate via the verify_driver_pin RPC (security definer) and then
-- talk to PostgREST as anon. We allow narrow operations:
...
-- NOTE: For Phase 1 we accept that driver writes are gated only by the PIN RPC
-- on the client. To harden later, move driver writes behind security-definer
-- RPCs that require a fresh PIN-derived token.
